...
def get_thread_title(thread_id: str) -> str:
    ...
    # Fallback: try to derive title from first message in checkpointed state
    try:
        ...
        for msg in messages:
            if isinstance(msg, HM):
                title = generate_title(msg.content if isinstance(msg.content, str) else str(msg.content))
                _THREAD_TITLES[tid] = title
                return title
    except Exception:
        pass
...
def delete_thread(thread_id: str) -> bool:
    """
    Delete all checkpoints for a thread from SQLite and clean up in-memory state.
    ...
    try:
        ...
        # SqliteSaver uses tables: checkpoints, checkpoint_blobs, checkpoint_writes
        # All rows are keyed by (thread_id, ...) or stored in a JSON config column.
        # LangGraph SQLite stores thread_id in the 'thread_id' column.
